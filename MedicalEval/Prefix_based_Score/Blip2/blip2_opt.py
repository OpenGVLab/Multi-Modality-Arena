...
class Blip2OPT(Blip2Base):
    ...
    def forward_lm(self, samples):
        """
        calculate ppl of the located token as OPT
        """
        image = samples["image"]
        with self.maybe_autocast():
            image_embeds = self.ln_vision(self.visual_encoder(image))
    ...
    @torch.no_grad()
    def forward_lm_without_image(self, samples, device):
        """
        calculate ppl of the located token as OPT, with pure text input
        """
        with self.maybe_autocast():
            self.opt_tokenizer.padding_side = "right"
