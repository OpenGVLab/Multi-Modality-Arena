...
class MplugOwlForConditionalGeneration(MplugOwlPreTrainedModel):
    ...
    def forward_lm(
        ...
        **generate_kwargs,
        ...
        #print("input_ids",input_ids.shape)  #[10,121]
        ...
        #print("inputs_embeds",inputs_embeds.shape) #[228, 3, 4096] [10,121,4096]
        ...
        labels[:,:start_loc] = -100
        labels[:,start_loc+lang_diff:] = -100

        loss = []
        for i in range(len(labels)):
            input_embeds_i = inputs_embeds[i,:,:].unsqueeze(0) #[10,122,4096]
        ...
        loss = np.reciprocal(loss)
        loss = loss.reshape(-1,1)
        '''
        attention_mask [10,122]
        print("labels",labels.shape) #[228, 3] [10,121]
        outputs = self.language_model(
            inputs_embeds=inputs_embeds,
            attention_mask=attention_mask,
            labels=labels,
