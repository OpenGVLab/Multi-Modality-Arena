...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        VLP_web_data_path = os.path.join(os.path.dirname(pathlib.Path(__file__).parent.parent.absolute()), 'VLP_web_data')
        tokenizer_path = os.path.join(VLP_web_data_path, 'llama_checkpoints', 'tokenizer.model')
        llama_7b_dir = os.path.join(VLP_web_data_path, 'llama_checkpoints', '7B')
        print(llama_7b_dir)
...
#        model.load_state_dict(adapter_weight, strict=False)
    ...
    def forward_lm(
        ...
        labels = tokens[:,0:start_pos]#tensor([[    1,  1724, 29915, 29879,   278,  2927,   310,   263,  3006,   305,29973,  4628, 29889]], device='cuda:5')
        labels = labels.detach().cpu().numpy()
        labels[0, :start_loc] = -100
        labels[0, start_loc + lang_diff:] = -100  
