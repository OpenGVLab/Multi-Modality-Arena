...
# #            prompt_k = self.wk(prompt).view(1, prompt_len, self.n_local_heads, self.head_dim).repeat(bsz, 1, 1, 1)
# #            prompt_v = self.wv(prompt).view(1, prompt_len, self.n_local_heads, self.head_dim).repeat(bsz, 1, 1, 1)
#             xk = torch.cat([prompt_k, xk], dim=1)
#             xv = torch.cat([prompt_v, xv], dim=1)
#             extra_mask = torch.zeros(1, 1, seqlen, prompt_len).to(mask)
...
#         if prompt is not None:
#             scores = torch.cat([self.gate.tanh().half() * self.new_gate * F.softmax(scores[:, :, :, :prompt_len].float(), dim=-1).type_as(xq), F.softmax(scores[:, :, :, prompt_len:].float(), dim=-1).type_as(xq)], dim=-1)
#         else:
